...
class NcnnTiledCancellationTest {
    @get:Rule
    val temporaryFolder = TemporaryFolder()

    @Test
    fun cancellationBetweenTilesStopsBeforeNextNativeInference() {
        lateinit var engine: NcnnUpscaleEngine
        val native = CancellingFakeNcnnNativeApi(scale = 4) { call ->
            if (call == 1) {
                engine.cancel()
            }
        }
    ...
    private class CancellingFakeNcnnNativeApi(
        private val scale: Int,
        private val afterInference: (Int) -> Unit,
    ) : NcnnNativeApi {
        var inferCalls = 0
        var cancelCalls = 0
        ...
        override fun infer(
            ...
            val outputRowStride = outputWidth * 4
            var offset = 0
            ...
            afterInference(inferCalls)
            return NcnnNativeInferenceResult(
                errorCode = NcnnNativeError.NONE,
                outputWidth = outputWidth,
                outputHeight = outputHeight,
                outputRowStrideBytes = outputRowStride,
                gpuUsed = true,
            )
        }
