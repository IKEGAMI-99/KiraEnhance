...
class NcnnTiledProgressTest {
    @get:Rule
    val temporaryFolder = TemporaryFolder()

    @Test
    fun progressTracksCompletedTilesDuringTiledInference() {
        lateinit var engine: NcnnUpscaleEngine
        val snapshots = mutableListOf<UpscaleProgress>()
        val native = ProgressRecordingNativeApi(scale = 4) {
            snapshots += engine.progress()
        }
        engine = NcnnUpscaleEngine(native)
        engine.load(validRequest())

        val result = engine.upscale(
            input = rgbaInput(width = 33, height = 33),
            settings = UpscaleSettings(outputScale = 4, tileSize = 32),
        )

        assertTrue(result is UpscaleResult.Success)
        assertEquals(
            listOf(
                UpscaleProgress(completedTiles = 0, totalTiles = 4),
                UpscaleProgress(completedTiles = 1, totalTiles = 4),
                UpscaleProgress(completedTiles = 2, totalTiles = 4),
                UpscaleProgress(completedTiles = 3, totalTiles = 4),
            ),
            snapshots,
        )
        assertEquals(
            UpscaleProgress(completedTiles = 4, totalTiles = 4),
            engine.progress(),
        )
    }
    ...
    private class ProgressRecordingNativeApi(
        private val scale: Int,
        private val beforeInfer: () -> Unit,
    ) : NcnnNativeApi {
        ...
        override fun infer(
            ...
            repeat(outputWidth * outputHeight) {
                outputPixels.put(offset, 0x44)
                outputPixels.put(offset + 1, 0x55)
                outputPixels.put(offset + 2, 0x66)
